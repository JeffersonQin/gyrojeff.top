...
$$
	\exist N, s.t. ~n > N \Rightarrow A - \epsilon < \sum_{k=0}^n a_k < A + \epsilon
$$

取 $\epsilon = A - A'$，则

$$
	\exist N, s.t. ~n > N \Rightarrow A - (A - A') < \sum_{k=0}^n a_k < A + (A - A')
$$
...
$$
	\sum_{k\in K} a_k = \sum_{k\in K} \real a_k + i\sum_{k\in K} \image a_k
$$

根据之前实数的定义：

$$
	\sum_{k\in K} a_k = \sum_{k\in K} \real a_k^+ - \sum_{k\in K} \real a_k^- + i\sum_{k\in K} \image a_k^+ - i\sum_{k\in K} \image a_k^-
$$
