...
1. $\displaystyle 2^k \Big \lfloor \frac {n+2^{k-1}} {2^k} \Big \rfloor \in \mathbb Z$
2. $\displaystyle t_k(n) \in (-2^{k-1},2^{k-1}]$ (可以很容易地通过讨论 $n$ 是否小于等于 $2^{k-1}$ 证明)

此外，不难看出，对于没有改变，即正常的项，我们定义的 $t_k(n)$ 仍旧使用，因为此时 $t_k(n) = n$。
